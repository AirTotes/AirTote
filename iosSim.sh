...
dotnet command will executed like below ...
	dotnet build -t:Run ./FIS-J/FIS-J.csproj -f net6.0-ios -r iossimulator-x64 --no-self-contained --nologo "/p:_DeviceName=[DeviceName|:v2:udid=UDID]" [dotnet command options]
...
if [ ! -z "$DeviceName" ]; then
  dotnet build -t:Run ./FIS-J/FIS-J.csproj -f net6.0-ios -r $RUNTIME_IDENTIFIER --self-contained --nologo "/p:_DeviceName=$DeviceName" ${ARGS[@]}
fi
